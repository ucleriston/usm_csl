...
DIR_REVISOES = os.path.join(RAIZ, "revisoes")
DIR_CONFIG_MOTOR = os.path.join(RAIZ, "config")
DIR_MODELO = os.path.join(RAIZ, "modelos", "revisao-modelo")
...
class Revisao:
    ...
    def criar(cls, slug, titulo="", tema="", base=None):
        ...
        if not os.path.isdir(DIR_MODELO):
            raise FileNotFoundError("esqueleto ausente: %s" % DIR_MODELO)
