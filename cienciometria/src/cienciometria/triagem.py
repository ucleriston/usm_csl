...
def contagens_prisma(registros_brutos, estatisticas_dedup, linhas_triagem=None, codigos=None):
    """Monta as contagens do fluxo PRISMA a partir do pipeline.

    `codigos` são os códigos de exclusão declarados no protocolo da revisão;
    na falta deles, valem os padrões de modelo.CODIGOS_EXCLUSAO.
    """
    codigos = codigos or CODIGOS_EXCLUSAO
    ...
    if linhas_triagem:
        ...
        fluxo["exclusoes_por_codigo"] = {
            c: {"n": n, "descricao": codigos.get(c, "código não previsto no protocolo")}
            for c, n in sorted(contagem_codigos.items())
        }
        sem_codigo = fluxo["exclusoes_por_codigo"].get("", {}).get("n", 0)
        if sem_codigo:
            fluxo["alerta"] = "%d exclusões sem código do protocolo" % sem_codigo
    return fluxo
