...
def montar(resultados, dir_saida, dados_execucao=None):
    ...
    identificacao = r.get("revisao") or {}
    add("# Relatório cienciométrico — %s\n" % (identificacao.get("titulo") or "revisão sem título"))
    if identificacao.get("tema"):
        add("**Tema:** %s  " % identificacao["tema"])
    add("**Revisão:** `%s`  " % (identificacao.get("slug") or "—"))
